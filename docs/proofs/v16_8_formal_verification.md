...
- Boundary entropy \(\frac{\text{Area}}{4G}\): 1.493648e+52 (scaled proxy)  
- Relative discrepancy: < 10⁻⁵⁰ (within cosmic variance)
...
시간 도함수:
\[
\dot{V} = p \dot{p} + S \dot{S} = p ( - S) + S ( - |p|) = -S|p| - S|p| = -2S|p| \le 0
\]
